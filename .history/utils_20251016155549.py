...
class BinaryNode:
    ...
    def __str__(self):
        ...
        # Store root reference for _nodeRepr to access
        self._tempRoot = self
        ...
        while f:
            ...
            el._tempRoot = self  # Pass root reference
        ...
        # All rows have the same length: 2^(maxLevel-1) nodes with "  " separators
        # Calculate based on actual content
        sampleRow = "  ".join(toPrint[maxLevel])
        rowLen = len(sampleRow)
        
        res = ""
        for level in range(1, maxLevel + 1):
            row = "  ".join(toPrint[level])
            res += row.center(rowLen) + "\n"
        
        # Clean up temp reference
        delattr(self, '_tempRoot')
...
class FrequencyNode(BinaryNode):
    ...
    def _nodeRepr(self):
        ...
        # Get maxVal from root to ensure consistent width
        root = self
        while hasattr(root, '_tempRoot'):
            root = root._tempRoot
        
        # Empty nodes should display as spaces with same width as real nodes
        if not self:
            # Calculate the width a real node would take
            valLen = len(str(1))  # Minimum width
