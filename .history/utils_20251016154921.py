...
class BinaryNode:
    ...
    def __str__(self):
        ...
        while f:
            ...
            # Only process children if this is not an empty node and we're not at max level
            if el and itsLevel < maxLevel:
                if el.left:
                    f.append(el.left)
                else:
        ...
        for level in range(1, maxLevel + 1):
            rowLen = len(toPrint[level]) * len(toPrint[level][0]) + 2 * len(toPrint[level])
            res += ("  ".join(toPrint[level])).center(rowLen) + "\n"
