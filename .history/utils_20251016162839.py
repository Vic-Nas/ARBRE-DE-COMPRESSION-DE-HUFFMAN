...
class BinaryNode:
    ...
    def __str__(self):
        ...
        minSpacing = 2  # Minimum spacing between nodes
        
        # Build from bottom up - calculate positions for each node
        positions = defaultdict(list)  # positions[level] = list of column positions
        
        # Bottom level: nodes are evenly spaced
        bottomNodes = toPrint[maxLevel]
        currentPos = 0
        for node in bottomNodes:
            positions[maxLevel].append(currentPos)
            currentPos += nodeWidth + minSpacing
        
        # Work upward: each parent is centered between its two children
        for level in range(maxLevel - 1, 0, -1):
            childPositions = positions[level + 1]
            for i in range(len(toPrint[level])):
                leftChildPos = childPositions[i * 2]
                rightChildPos = childPositions[i * 2 + 1]
                # Parent is centered between left and right child
                parentPos = (leftChildPos + rightChildPos + nodeWidth) // 2
                positions[level].append(parentPos)
        
        # Now build the output using the calculated positions
        totalWidth = positions[maxLevel][-1] + nodeWidth
        res = ""
        
        for level in range(1, maxLevel + 1):
            line = [" "] * totalWidth
            for i, node in enumerate(toPrint[level]):
                pos = positions[level][i]
                # Place the node at its position
                for j, char in enumerate(node):
                    if pos + j < totalWidth:
                        line[pos + j] = char
